...
bot = Discordrb::Bot.new token: 'トークン'
puts "This bot's invite URL is #ここにDiscordの招待URL."
...
bot.message(content: 'plz atk') do |event|
  event.respond "#{event.user.name}:#{attack[rand(0..attack.length)]}"
end

#"plz defense"とチャットに入力があった場合反応
bot.message(content: 'plz def') do |event|
  event.respond "#{event.user.name}:#{defense[rand(0..defense.length)]}"
end

#"plz character"とチャットに入力があった場合反応
bot.message(content: 'plz character') do |event|
  event.respond "#{event.user.name}:#{data[rand(0..data.length)]}"
end
